...
;; Reads casefile and returns a map with Case and Vignette as key and list of Operations as vals
...
;; Returns map of requiremnt file where SRC is the key and string of quantity is the value
...
(def branches
  (zipmap ["01" "02" "03" "05" "06" "07" "08" "09" "10" "11" "12" "14" "16" "17" "19" "20" "27" "30" 
           "31" "33" "34" "35" "37" "40" "41" "42" "43" "44" "47" "51" "52" "53" "55" "63" "77" "87" "90"]
          ["Aviation" "Band" "Chemical" "Engineer" "Fires" "Infantry" "Medical" "Ordance" "Quartermaster" "Signal" "Human Resources" "Finance"
           "Chaplaincy" "Cavalry" "Military Police" "Military History" "Judge Advocate General" "Military Intelligence" "Special Forces" "MISO"
           "Military Intelligence" "Interpreters" "CBRNE" "Space" "Civil Affairs" "Quartermaster" "Maintenance" "Air Defense" "Public Affairs" 
           "Stryker Brigade Combat Team" "Mission Command" "Corps Headquarters" "Information Operations" "Transportation" "Sustainment"
           "Infantry Brigade Combat Team" "Armor Brigade Combat Team and Divisions" "Contracting Branch"]))
;; =============================================================================

(defn get-srcs [lines header] (set (map #(nth % (get header "SRC")) lines))) ;; Returns set of SRCs from demand or supply file given lines and the correct header
(defn get-branch-code [src] (apply str (take 2 src))) ;; Returns the Branch Code of the 9 digit SRC, where branch code is the first two digits
(defn get-branch [branch-code branches] (get branches branch-code)) ;; Returns the English Definition of the branch code from the branches map
(defn get-title [src slines sheader] (nth (first (filter #(= src (nth % (get sheader "SRC"))) slines)) (get sheader "OITitle"))) ;; Returns the Title of the SRC   
(defn in-demand? [ssrc dsrcs] (get dsrcs ssrc)) ;; Returns non-nil when the Supply SRC is in the Demand file

;; Returns non-nil when the SRC is exclusivly in the union of RC and NG (ie SRC is in RC or NG but not AC) 
(defn rc-only? [src supply supplyheader]
  (and (not (get (get-srcs (get supply "AC") supplyheader) src)) 
       (get (get-srcs (concat (get supply "RC") (get supply "NG")) supplyheader) src)))

(defn get-src-cost [src costs] (if (get costs src) (get costs src) 0)) ;; Returns cost of one unit of SRC  

;; Returns the total quantity for the SRC in the supply file
(defn get-src-supply [src supply supplyheader]
  (apply + (map read-string (map #(nth % (get supplyheader "Quantity")) (filter #(= src (nth % (get supplyheader "SRC"))) (set (apply concat (vals supply))))))))

(defn get-ARNG-supply [src supply supplyheader] (apply + (map read-num (map #(nth % (get supplyheader "Quantity")) (filter #(= src (nth % (get supplyheader "SRC"))) (get supply "NG"))))))
(defn get-USAR-supply [src supply supplyheader] (apply + (map read-num (map #(nth % (get supplyheader "Quantity")) (filter #(= src (nth % (get supplyheader "SRC"))) (get supply "RC"))))))
(defn get-RC-supply   [src supply supplyheader] (+ (get-USAR-supply src supply supplyheader) (get-ARNG-supply src supply supplyheader)))
(defn get-AC-supply   [src supply supplyheader] (apply + (map read-num (map #(nth % (get supplyheader "Quantity")) (filter #(= src (nth % (get supplyheader "SRC"))) (get supply "AC"))))))
...
(defn get-src-personnel [src supply supplyheader]
  (apply + (map #(read-num (nth % (get supplyheader "Strength"))) 
             (filter #(= src (nth % (get supplyheader "SRC"))) (apply concat (vals supply))))))

;; Returns the total personnel for the SRC by multiplying the quantity of the SRC by the personnel per SRC
(defn get-src-total-personnel [src supply supplyheader]
  (apply * (map #(% src supply supplyheader) [get-src-personnel get-src-supply])))

(defn get-requirement [src reqs ] (if (get reqs src) (read-num (get reqs src)) 0)) ;;Returns quantity required (0 if does not exist)
(defn get-requirement-cost [src req cost] (* req (get-src-cost src cost))) ;; Returns the cost for the required number of SRCs
(defn get-requirement-personnel [src req supply supplyheader] (* req (get-src-personnel src supply supplyheader))) ;; Returns the personnel of the required quantity of SRCs

(defn supply->lines [supply supplyheader demand demandheader reqs costs] ;;Formats output lines by calling the lookup functions
  (for [src (get-srcs (apply concat (vals supply)) supplyheader) :let [req (get-requirement src reqs)]]
    [(get-branch-code src) ;;Branch Code
     ...
     (* (get-src-cost src costs) (get-AC-supply src supply supplyheader)) ;;Total cost of SRCs in supply (supply * cost)
     (get-src-personnel src supply supplyheader) ;;Personnel per src 
     (* (get-src-personnel src supply supplyheader) (get-AC-supply src supply supplyheader)) ;;Total SRC personnel for AC
     req ;;Required units of SRC
     (if (= 0 req) (let [d (in-demand? src (get-srcs (get demand "TrueDemands") demandheader))] (if d "RC met demand" "No demand")) "") ;;Why no requirement?
     ...
     (- (* (get-src-cost src costs) (get-AC-supply src supply supplyheader)) (get-requirement-cost src req costs)) ;;Marginal Cost
     ( - (* (get-src-personnel src supply supplyheader) (get-AC-supply src supply supplyheader)) (get-requirement-personnel src req supply supplyheader))])) ;;Marginal Personnel

(defn post-process->lines [inputfile supplyfile demandfile costfile] ;;Reads in inputfiles and formats output lines
  (let [supply (read-supply supplyfile) supplyheader (read-header supplyfile)
        ...
        header (apply conj (into [] (sort-by #(get (read-header inputfile) %) (keys (read-header inputfile))))
                 ["Branch Code" "Branch" "Unit Grouping" "Supply SRCs" "Title" "In Demand?" "RC Only?" "ARNG Supply" "USAR Supply" "RC Supply" "AC Supply"
                  "AC O&S Cost/SRC" "AC Supply Cost" "Personnel/SRC" "AC Supply Personnel" "Requirements" "Why no Requirements?" "Requirement Cost"
                  "Requirement Personnel" "Supply - Requirement" "Additional Cost" "Additional Personnel"])]
      (conj (apply concat 
              (for [i input]
                (for [line (supply->lines supply supplyheader demand demandheader (read-reqs (:reqfile i)) costs)]
...
(defn output-post-process [outputfile lines]
  (with-open [w (clojure.java.io/writer outputfile)]
    (doseq [line lines]
      (doseq [val line] (.write w (if (= "0" (str val)) (str "-" "\t") (str val "\t")))) (.write w "\r\n"))))

(defn choose-file [title & dir-only] ;;GUI menu to select file or directory (returns string/seq of string (filenames))
...
(defn ->output [] ;;Ask user to select inputs then processes output file
  (let [supplyfile (first (choose-file "SELECT SUPPLY INPUT FILE (.TXT)"))
        ...
        inputfile  (first (choose-file "SELECT FILLED OUT TEMPLATE FILE (.TXT) *MAKE SURE THE FILEPATHS TO REQUIREMENT FILES HAS BEEN FILLED OUT."))
        outputfile (str (first (choose-file "SELECT OUTPUT DIRECTORY" true)) "/Requirement Analysis Comforter Output.txt")]
    (println "\n\tSupply File:\t" supplyfile "\n\tDemand File:\t" demandfile "\n\tCost File:\t" costfile "\n\tInput File:\t" inputfile "\n\tOutput File:\t" outputfile)
    (time (output-post-process outputfile (post-process->lines inputfile supplyfile demandfile costfile)))))
